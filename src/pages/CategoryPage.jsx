...
const CategoryPage = () => {
  ...
  async function fetchCatsAndCounts() {
    try {
      const [catRes, allBlogsRes] = await Promise.all([getCategories(), getBlogs()]);
            ...
            <aside className="lg:col-span-4 space-y-6">
              ...
              <div className="bg-white border border-gray-200 rounded-lg">
                <div className="flex">
                  {['Popular', 'Recent', 'Commented'].map((tab) => (
                    <button
                      ...
                      className={`flex-1 px-4 py-2 text-sm font-medium border-b ${activeTab === tab ? 'bg-gray-100 text-gray-900' : 'text-gray-600 hover:bg-gray-50'}`}
                    >
                      {tab}
                    </button>
                  ))}
                ...
                <div className="p-4 space-y-4">
                  {(activeTab === 'Popular' ? blogs : activeTab === 'Recent' ? [...blogs].reverse() : blogs)
                    .slice(0, 5)
                    .map((b) => (
                      <Link to={`/blog/${b._id}`} key={`${activeTab}-${b._id}`} className="flex gap-3 group">
                        ...
                        <div className="min-w-0">
                          <div className="text-sm font-semibold text-gray-800 line-clamp-2 group-hover:text-red-600">{b.title}</div>
                          <div className="text-xs text-gray-500 mt-1">{new Date(b.date).toLocaleDateString('en-GB', { day: '2-digit', month: 'short', year: 'numeric' })}</div>
                        </div>
                      </Link>
                    ))}
                </div>
                    ...
                    {categoriesList.map((cat) => (
                      <li key={cat._id}>
                        <Link
                          ...
                          className={`text-sm hover:text-blue-700 ${slug === cat.slug ? 'text-blue-700 font-medium' : 'text-gray-700'}`}
                        >
                          {cat.name} <span className="text-gray-500">({counts[cat._id] || 0})</span>
                        </Link>
                      </li>
                    ))}
                  </ul>
