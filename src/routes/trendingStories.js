...
router.get('/', async (req, res) => {
  try {
    ...
    const blogs = trendingStories.map(ts => ts.blogId).filter(blog => blog !== null);
...
router.post('/', async (req, res) => {
  try {
    ...
    if (existing) {
      return res.status(400).json({ message: 'Blog is already in trending stories' });
    }
...
router.delete('/:blogId', async (req, res) => {
  try {
    ...
    if (!result) {
      return res.status(404).json({ message: 'Blog not found in trending stories' });
    }
